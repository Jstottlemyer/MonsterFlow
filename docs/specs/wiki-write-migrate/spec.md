---
created: 2026-05-15
revision: V3 (post-V2-spec-review; Codex caught F2 algorithm self-defeating + F3 reconstruction under-spec'd; folded inline)
constitution: none
confidence:
  scope: 0.95
  ux: 0.92
...
**Created:** 2026-05-15
**Revision:** V3 — V2 went to /spec-review and Codex caught that V2's F2 idempotency algorithm was internally self-defeating (case-insensitive both-sets check skipped the canonical `[[Welcome]] → [[welcome]]` rewrite on pass 1) AND V2's F3 split-brain reconstruction was under-specified. V3 fixes both with exact-text-equality idempotency check + persistent pre-migration vault index sidecar. Also: lock spans Phase A+B (Codex), fsync before rename (Codex), iCloud `.icloud` placeholder detection (V1 F17 / V2 G-V2-3), CG-1 stderr contract pin, CG-2 archive-timestamp scope pin, SC-V2-003 drops whitespace-padded + nested wikilink cases. V2 also folded the V1 review findings (F1-F5 architectural + 11 contract refinements). F1 empirically verified via Obsidian spike at `~/Projects/_spike-f1-wiki-write-migrate/` (2026-05-16, all 5 link probes resolved purple).

...
20. ~~**`brew` unavailable when Obsidian.app needs installing**~~ — *Dropped in /check round 1 (design rev1, D11/MF1):* `brew` is REQUIRED tier in install.sh (line 342); a missing brew aborts install.sh before Knowledge Layer runs, so this edge case is unreachable from `do_knowledge_layer`'s insertion point. `detect_obsidian_app` emits only `ready` or `can-install` (no `manual` token); `print_obsidian_app_manual_instructions` is removed from the function surface. Documented direct-download fallback remains relevant to humans who hit the REQUIRED-tier check, but it lives in install.sh's REQUIRED-tier error message, not in the Knowledge Layer.

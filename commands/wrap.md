...
**Cold-start handling:** features with <3 runs in the current window render as `(insufficient data — N runs)` and are excluded from drift arrows. Empty rollup renders: `Persona drift: no measured features yet — run 1+ feature through /spec-review and /blueprint to seed.`

...
- **`wiki-write-conventions`** — deterministic helper at `scripts/wiki-write.py` for canonical Obsidian vault writes. Slug rule: Unicode-dash normalization (7 dash variants → ASCII hyphen-minus) + strict kebab-ASCII + 80-char cap, 8 pinned fixture cases. Layout: `projects/<slug>/index.md` + `<slug>/<topic>.md` (folder+index); `concepts/<slug>.md` and `entities/<slug>.md` flat. Helper owns the full write lifecycle (frontmatter + body, atomic via `tempfile.NamedTemporaryFile` + `os.replace`); accepts body via `--body <text>` or `--body-stdin` (mutually exclusive). Three modes: default-write, `--lint`, `--write-conventions <vault>`. Python 3.9-compatible (no `|` unions, no `match`, no parenthesized context managers — Codex P1 #4 caught the shebang-failure risk on adopter machines with system 3.9). 7-class exception hierarchy with `exit_code` attribute per class (0/1/2/3 exit code matrix; argparse errors collapse to 1).
- **`scripts/_replace_sentinel_block.py`** — standalone atomic sentinel-block replace/append/idempotent-skip utility. Used by `install.sh` for `~/CLAUDE.md` injection. CLI: `--stdin-content` or `--content-file <path>` (mutually exclusive), optional `--backup <path>` (only fires on actual content change — supports idempotent-no-op contract). Stands alone per memory `hook_stdin_heredoc` (never `python3 -` with stdin JSON).
- **`templates/wiki-conventions.md`** — single human-readable source of truth for slug rules, layout rules, frontmatter schemas per category, and Obsidian Settings → Excluded files manual step (Obsidian doesn't honor `exclude: true` natively; `_` prefix on filename is the actual exclusion).
- **`install.sh` two-stage convention install** (per Codex P1 #1 — `do_knowledge_layer` runs at install.sh:1421 BEFORE `claude-md-merge.py` at install.sh:1440; injecting the sentinel block earlier would let the baseline merge interact with or strip the sentinels): `_install_vault_conventions()` writes per-category `_convention.md` files inside `do_knowledge_layer` Zone B (vault writes are independent of CLAUDE.md merge); `_install_wiki_conventions_claude_md_block()` injects the `<!-- WIKI-CONVENTIONS-START -->` / `<!-- WIKI-CONVENTIONS-END -->` block into `~/CLAUDE.md` AFTER the baseline merge. Backup-dedupe via run-scoped `WIKI_CONV_CLAUDE_MD_BACKED_UP` guard; `--backup` flag passed to `_replace_sentinel_block.py` so backups only fire on actual content change (idempotent re-runs create zero `.bak.<epoch>` files).
- **`commands/wrap.md` Phase 2c Step 3b** — non-blocking convention lint. Runs `python3 ~/Projects/MonsterFlow/scripts/wiki-write.py --lint` in `/wrap` and `/wrap-insights` when vault is configured; skipped in `/wrap-quick` via existing Phase 2c top-level gate. Surfaces 4 violation types (Unicode-dash in filename, mixed-case in filename, projects flat-file 3a, projects folder-no-index 3b) as a warn block; exit code always 0 from the helper.
- **`tests/test-wiki-write.sh`** — 50-case harness across 8 sections: slugify fixtures (8 cases per AC #3 — pins both em-dash-with-spaces AND em-dash-without-spaces transforms), exception classes / exit codes (4), frontmatter shape per category (4), atomic-write fault injection (1), `_replace_sentinel_block.py` (5), `--lint` zero violations + 4 violation types + silent-skip (6), `--write-conventions` files + frontmatter + idempotency-backup (4), orchestrator-wiring grep assertion (1, covers ck-t7-grep per `test-orchestrator-wiring-gap` memory), body via flag and stdin + mutual exclusion (3), YAML omit-None + empty-tags (2), install + wrap integration (10).
- **Backlog entry `wiki-write-migrate`** — the migration half of the original `wiki-write-conventions` design, carved to follow-up after /spec-review V1 surfaced ~40-60% of implementation surface in migration alone (wikilink-rewriting shortest-unique-path, filesystem collision matrix, durable journal for resume/rollback). v0.16.0 handles new writes deterministically; existing wrong-structure pages get flagged by lint until the migrate spec ships.

### Fixed

- **`install.sh` idempotent re-run no longer creates `~/CLAUDE.md.bak.<epoch>`** — pre-existing AC3 in `test-install-knowledge-layer.sh` asserts zero backup files on the second install run. Initial v0.16.0 implementation eagerly backed up before invoking `_replace_sentinel_block.py`; fixed by passing `--backup` flag to the helper so backups only fire on actual content change. Mirrored fix at the pre-baseline-merge site (claude-md-merge.py manages its own backup-on-change behavior).

...
Uses the standard pipeline: `/kickoff` → `/spec` → `/spec-review` → `/blueprint` → `/check` → `/build` (`/plan` is Claude Code's built-in plan-mode, not MonsterFlow's design gate — use `/blueprint` for the pipeline)

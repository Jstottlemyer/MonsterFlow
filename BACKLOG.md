...
- **`token-economics-spike-q1-reopen` (spec candidate, S-M)** — re-open token-economics Spike Q1: 34 of the discovered persona dispatches have parent-annotation `total_tokens` != final-subagent-row broad sum, tripping the A1.5 hard exit in `scripts/compute-persona-value.py` and leaving `dashboard/data/persona-rankings.jsonl` empty (which silently starves `/wrap-insights` Phase 1c and `agent_budget` persona selection). Rankings were regenerated 2026-06-12 with the sanctioned `--best-effort` downgrade, so the pipeline is unblocked, but the root cause (likely a harness change in how parent sessions annotate subagent token usage) is unconfirmed. **Why:** `--best-effort` is a standing data-quality waiver; until Q1 is re-validated, per-persona cost figures may be systematically off, and cost is one of the ranking inputs. **Entry points:** `scripts/compute-persona-value.py` `a15_crosscheck()`; sample mismatched dispatches from `~/.claude/projects/*/...` to characterize the delta (off-by-cache-creation? new usage keys?). **Size:** S-M (diagnosis-heavy, small code change). **Sequencing:** unblocked; should land before trusting `cheapest per call` rankings for budget tuning.

---

## Captured 2026-05-17 (session ideas)

- **`wrap-fewer-permission-prompts-integration` (spec candidate, S)** — wire Anthropic's `/fewer-permission-prompts` skill into `/wrap-full` as a new Phase 4b (permission-allowlist tune). Gated to fire only when last-run was >14 days ago OR session count is >30 since last run (matches the existing soft-skip pattern in Phase 2b/5). Surfaces a one-line diff ("3 new auto-allow candidates: `cat`, `gh pr view`, `mcp__google_drive__list_recent_files` — accept?"), confirms once, writes to `.claude/settings.json`. **Why:** today the only path to a tuned allowlist is the user remembering to run `/fewer-permission-prompts` manually every few weeks. Folding it into the existing `/wrap` cadence converts "manual audit I keep forgetting" into "automatic biweekly review at session boundaries" without polluting every `/wrap`. **Architectural overlap:** both `/wrap` and `/fewer-permission-prompts` are session-history → derived-config flows. `/wrap` Phase 1a already reads `~/.claude/usage-data/session-meta/` — same data source `/fewer-permission-prompts` walks — so the integration cost is one Python call plus a sentinel file (e.g., `~/.claude/.fewer-permission-prompts-last-run`). **Size:** S (~50 LoC in `commands/wrap-full.md` or new Phase 4b block in `commands/wrap.md` gated on `args=full`). **Entry point:** `/spec wrap-fewer-permission-prompts-integration` when ready. **Sequencing:** unblocked. Should also bake adopter-friendly baseline into MonsterFlow's `settings/settings.json` so the floor is universal and `/fewer-permission-prompts` adds the per-user ceiling.
...
- **`autoship-outcome-instrumentation` (spec candidate, S)** — wire the v1-deferred `outcome` event row into the autorun post-merge flow. Currently `scripts/_goal_autoship_render.py log-event --event-type outcome --reason shipped --pr N` works (helper API contract is tested) but no skill emits it at runtime. Post-merge logic lives in `scripts/autorun/run.sh`; add a hook there that calls the helper after a successful merge. Closes the self-learning loop — without outcome rows, per-tag ship-rate analysis (`autorun-suitability-v2`) can't see actual ship outcomes. **Why deferred from autonomous-shipping-defaults V3:** Codex caught at /blueprint Phase 2b that `commands/build.md` has no post-merge logic; clean carve. **Size:** S (~30 LoC in run.sh + 1 test case). **Sequencing:** prerequisite for `autorun-suitability-v2` evidence-driven heuristic tuning. **Entry point:** `/spec autoship-outcome-instrumentation` after autonomous-shipping-defaults v1 has ~20 specs of render+halt data.

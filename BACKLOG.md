...
## From wiki-write-conventions /spec-review V2 carve-out (2026-05-15)

- **`wiki-write-migrate` (spec candidate, M)** — the migration half of the original `wiki-write-conventions` design, carved to a follow-up after /spec-review V1 surfaced 3 Critical reviewer findings + Codex findings #3, #4, #5 around mid-run rollback / filesystem-collision matrix / wikilink shortest-unique-path resolution. v1 (`wiki-write-conventions`, shipped 2026-05-?) handles new writes deterministically; existing wrong-structure pages get flagged by lint but stay in place. Migration is the cleanup pass. **Why deferred:** ~40-60% of the v1 design's implementation surface lived in migration. Wikilink rewriting in Obsidian is harder than basename-only substitution (Codex finding #5 — multiple files can share basenames in different folders; shortest-unique-path is the resolver model, not literal-string match). Filesystem collisions on case-insensitive macOS HFS+/APFS plus Unicode normalization (Codex finding #4) need a proper collision matrix that wasn't in scope tonight. Rollback-vs-resume-vs-no-revert needs a durable journal design (gaps:opus #1, Codex #3). **Size:** M. Touches: `scripts/wiki-write.py` (gains `--migrate [--dry-run] [--resume]` subcommand, ~150 LoC + journal logic), `scripts/_wiki_migrate.py` (the rewriter and resolver, ~200 LoC), `<vault>/migration-report.md` (output artifact), `<vault>/.migration-journal.json` (durable state for resume), `tests/test-wiki-migrate.sh` (new harness). **Open questions for `/spec`:** (1) wikilink resolver model — shortest-unique-path (Obsidian native) or full-path (`[[projects/welcome/index|Welcome]]`)? (2) rollback strategy — durable journal + explicit `--resume` (verbose, correct) or no-revert + "user's responsibility" (simple, sharper edges)? (3) collision matrix — does the dry-run reject the whole migration or migrate-around the collisions? (4) aliases-in-frontmatter — preserve old human-visible titles via `aliases: [...]` to maintain discoverability after rename (Codex finding #13)? **Entry point:** `/spec wiki-write-migrate` after v1 has been in use for a week and we have empirical signal on what the lint surface actually reports for typical drift.

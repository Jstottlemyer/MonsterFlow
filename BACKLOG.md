...
- **`install-obsidian-vault-baseline` (spec candidate, S)** — ship a starter vault skeleton at `templates/obsidian-vault-baseline/` and have `install_obsidian_env` scaffold it to `OBSIDIAN_VAULT_PATH` when the target directory either doesn't exist or is empty (no `.obsidian/`, no `.manifest.json`, no other content). Adopter sees a working vault with the directory structure the 6 wiki skills expect (a top-level `README.md` explaining the wiki, empty `projects/` and `_raw/` subdirs with `.gitkeep`, an initialized `.manifest.json`, optionally a starter `index.md`). Tool clone vs vault baseline are independent: an adopter could clone the tool repo with their own custom vault, or take the baseline vault and never clone the tool repo, or both. **Critical privacy guard:** baseline files are pure template content authored in MonsterFlow — never anyone's actual notes. Adopter's vault becomes theirs the moment Obsidian.app opens it. **Why:** today even after `brew install --cask obsidian` + `OBSIDIAN_VAULT_PATH` config, the adopter still hits "what do I put here?" — Obsidian.app's "create new vault" flow makes an empty dir, and the wiki skills assume conventions the user has to discover. A 5-file baseline removes that confusion. **Size:** S (template-content authoring + a `scaffold_obsidian_vault_baseline` helper with idempotency guard "don't overwrite non-empty vault" + 2-3 test cases: cold scaffold, idempotent re-run, refuse-to-clobber-non-empty). **Sequencing:** independent of `install-obsidian-wiki-auto-clone`; can ship before or after. **Entry point:** `/spec install-obsidian-vault-baseline` next session.
